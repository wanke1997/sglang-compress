# R-KV / SnapKV on SGLang — Findings & Optimization Roadmap
...
Branch: `feature/snapkv-conditional-summarization`.
...
(`benchmark/data/stage1_summarization.jsonl`); gold in `stage1_dev.jsonl` (the
runner strips trailing assistant turns → never leaks into the prompt).

---

## 1. What exists today (committed)

Three compressors, all sharing the same "compress the prompt at the end of
prefill, then decode normally" shape except decode R-KV:

| Compressor | File | Compaction timing | Scoring |
|---|---|---|---|
| **SnapKV** | `snapkv/integration.py` | prefill end | attention importance (cheap) |
...
- `a7620c8d4` overlap enablement + **retract KV-leak fix** (`prompt_len = req.seqlen`) + retract hooks for SnapKV/R-KV-prefill
- `43a41c131` **compression-aware admission** for prompt-phase (SnapKV / R-KV-prefill)
- `fe1b26d29` **re-gate overlap off** for prompt-phase (allocator race fix)

Constraints (all three prompt-phase configs): `--disable-radix-cache`,
`--disable-prefill-cuda-graph`, `--disable-overlap-schedule`, `--page-size 1`,
`tp==1`. Decode CUDA graph **is** supported and stays on.
...
- Budget scan (earlier, same judge): R-KV(A/B) ≈ SnapKV at low/mid budget, R-KV(A)
  best near budget 4096 (near-lossless); R-KV(B, buffered) best at mid budget
  (1024–2048). n=136 single-pass judge is noisy (~±2 rows).

### 2.2 Compression-aware admission — the throughput unlock
Memory-bound regime (64K pool, ~5k prompts, strict mem-check):
- Peak concurrent running-req: **Full KV 10 → SnapKV / R-KV-prefill 65 (6.5×)**.
- Throughput (conc 64, 1024 out): **SnapKV 1.91× (6001 vs 3137 tok/s)**;
  R-KV-prefill ≈ even at 5k prompts.
...
### 2.6 CUDA graph
Decode graph works for all three (logical rotary positions restored at
...
- Add a prompt-length × budget × (Full / SnapKV / R-KV-A / R-KV-B) table with
  throughput + quality.

### P3 — True mid-prefill physical KV release  (unlocks input > KV pool)
All three compressors compress at prefill **end**, so the *full* prompt KV is
...
sample for stronger quality claims (esp. A-vs-B and SnapKV-vs-R-KV, which flip
within noise at some budgets).

---

## 4. Known limitations / gotchas
- Overlap OFF is **required** for all three (allocator race — §2.5).
