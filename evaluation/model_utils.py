...
@torch.no_grad()
def generate_completions_method(model, tokenizer, prompts, batch_size=1, stop_id_sequences=None, add_special_tokens=True, disable_tqdm=False, **generation_kwargs):
    generations = []
    if not disable_tqdm:
        progress = tqdm.tqdm(total=len(prompts), desc="Generating Completions")
...
def load_hf_lm_and_tokenizer_method(
    ...
    if compression_method != None:
        if compression_method.lower() in ["snapkv", "snapkv_step", "snapkv_step_sim", "snapkv_step_sim_v2", "snapkv_step_sim_v3", "h2o"]:
            window_sizes = 8
        ...
        for idx_l in range(layers):
            ...
            model.model.layers[idx_l].self_attn.config.max_capacity_prompt = max_capacity_prompts[idx_l]
