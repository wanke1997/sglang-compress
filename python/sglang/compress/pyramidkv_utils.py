...
def compute_attention_scores(query_states, key_states, head_dim):
    ...
    _, k_heads, _, _ = key_states.shape
    ...
    if ratio == 1:
        ...
        # 重塑query_states以进行分组计算
        # 新形状: [batch_size, k_heads, ratio, seq_len, head_dim]
        query_states = query_states.view(batch_size, k_heads, ratio, seq_len, head_dim)
        
        # 扩展key_states以匹配query_states的维度
        # [batch_size, k_heads, 1, seq_len, head_dim]
        key_states = key_states.unsqueeze(2)
        
        # 计算attention scores并在ratio维度上取平均
        # [batch_size, k_heads, ratio, seq_len, seq_len] -> [batch_size, k_heads, seq_len, seq_len]
...
class SnapKVCluster():
    ...
    def update_kv(self, key_states, query_states, value_states, attention_mask, num_key_value_groups = None):
        ...
        # print(f"q_len {q_len} self.max_capacity_prompt {self.max_capacity_prompt}")
        

        if k_len < self.max_capacity_prompt:
            ...
            attn_weights = compute_attention_scores(query_states, key_states, self.window_size)
            ...
            # print(f"debug attn_weights {attn_weights.shape}")
            ...
            if self.merge is not None:
                key_states, value_states = merge_kv(key_states, value_states, indices, window_size, self.merge)
                return key_states, value_states
...
class H2OKVCluster():
    ...
    def update_kv(self, key_states, query_states, value_states, attention_mask, num_key_value_groups = None):
        ...
        if k_len < self.max_capacity_prompt:
            ...
            attn_weights = compute_attention_scores(query_states, key_states, window_size)
            # attn_weights = torch.matmul(query_states, key_states.transpose(2, 3)) / math.sqrt(head_dim)
            mask = torch.full((window_size, window_size), torch.finfo(attn_weights.dtype).min, device=attn_weights.device)
            mask_cond = torch.arange(mask.size(-1), device=attn_weights.device)
            mask.masked_fill_(mask_cond < (mask_cond + 1).view(mask.size(-1), 1), 0)
